...
import { displayTitle } from "@/data/stories";
...
export default function StoryRow({ story }: { story: Story }) {
  ...
  if (story.publicationStatus === "draft") {
    return (
      <div className="flex items-baseline gap-6 border-b border-line py-8 text-ink-soft/60">
        ...
        <div>
          <p className="text-[15px] sm:text-base">{title}</p>
  ...
  if (story.publicationStatus === "unverified") {
    return (
      <div className="flex items-baseline gap-6 border-b border-line py-8 text-ink-soft/60">
        <span className="w-10 shrink-0 text-2xl tabular-nums sm:text-3xl">
          {num}
        ...
        <div>
          <p className="text-[15px] sm:text-base">{title}</p>
          <p className="mt-2 text-[12px] tracking-wide">確認中</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
          ...
          {story.publishedAt && (
            <p className="shrink-0 text-[12px] tracking-wide text-ink-soft">
              {story.publicationStatus === "scheduled"
